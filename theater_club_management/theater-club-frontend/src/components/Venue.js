...
const Members = () => {
  ...
  const [newMember, setNewMember] = useState({ venue_id: '', venue_name: '', venue_types: ''});
  const { user } = useContext(UserContext);

  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/venue');
  ...
  const updateMember = async (id, updatedMember) => {
    try {
      ...
      formData.append('venue_name', updatedMember.venue_name);
      formData.append('vemuw_types', updatedMember.venue_types);
